...
-- ---- nfit_users (SMS opt-in state) ---------------------------------------
--
-- Light user-state table keyed on the email NextAuth gives us. Intentionally
-- minimal: we don't shadow NextAuth's session storage, we just persist the
-- bits NextAuth's JWT can't (phone, consent timestamps).
--
-- Why `nfit_users` and not `users`: avoids future collision if NextAuth's
-- Postgres adapter is added later, which would want to own `users` itself.
CREATE TABLE IF NOT EXISTS nfit_users (
  email                TEXT PRIMARY KEY,
  phone                TEXT,                     -- E.164 ("+15555551212")
  phone_verified_at    TIMESTAMPTZ,
  sms_consent_at       TIMESTAMPTZ,              -- when checkbox was ticked
  sms_opt_out_at       TIMESTAMPTZ,              -- when STOP came in
  created_at           TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at           TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS nfit_users_phone_idx
  ON nfit_users (phone) WHERE phone IS NOT NULL;

DROP TRIGGER IF EXISTS nfit_users_updated_at ON nfit_users;
CREATE TRIGGER nfit_users_updated_at
  BEFORE UPDATE ON nfit_users
  FOR EACH ROW EXECUTE FUNCTION set_updated_at();

-- ---- sms_verification_codes ----------------------------------------------
--
-- One active code per user (PK on email). Code stored as SHA-256 hash; the
-- short TTL (10 min) makes that hash strength sufficient for a 6-digit pin.
-- `attempts` is incremented on every failed verify and we cap at 5.
...
-- ---- sms_messages (audit log) --------------------------------------------
--
-- Full inbound+outbound log. Required by Twilio toll-free verification:
-- they want evidence we honor STOP/HELP and don't message after opt-out.
-- `email` is nullable because an unknown number can text in.
CREATE TABLE IF NOT EXISTS sms_messages (
  id                   BIGSERIAL PRIMARY KEY,
  email                TEXT,
  phone                TEXT NOT NULL,
  direction            TEXT NOT NULL CHECK (direction IN ('outbound', 'inbound')),
  kind                 TEXT NOT NULL,            -- verification | welcome | reminder | reply | stop | help | start | unknown
  body                 TEXT NOT NULL,
  twilio_sid           TEXT,
  created_at           TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE INDEX IF NOT EXISTS sms_messages_email_idx ON sms_messages (email, created_at DESC) WHERE email IS NOT NULL;
