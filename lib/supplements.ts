...
export interface EquipmentVariantOption {
  id: string;
  label: string;
}

export interface EquipmentVariantData {
  isCable?: boolean;
  variants: EquipmentVariantOption[];
  variantSetup?: Record<string, string>;
  superset?: SupersetInfo;
  variantSuperset?: Record<string, SupersetInfo>;
  variantSupersetNotes?: Record<string, string>;
}

export const CABLE_VARIANTS: EquipmentVariantOption[] = [
...
export const EQ_VARIANTS: Record<string, EquipmentVariantData> = {
  "SL Leg Press (Right)": {
    variants: [
  ...
  "SL Leg Extension (Right)": {
    ...
    superset: {
      ...
      safety:
        "Hip stays static and reclined, pure knee extension, zero iliopsoas.",
    },
  ...
  "Prone Ham Curl (Right)": {
    ...
    variantSuperset: {
      ...
      prone: {
        ...
        safety:
          "Force across knee only. Prone position keeps hip completely neutral.",
      },
      "plate-loaded": {
        title: "Left Leg Curl",
        sets: "3\u00D710",
